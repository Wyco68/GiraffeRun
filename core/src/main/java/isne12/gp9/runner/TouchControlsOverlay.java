 ...
 * Input handled here so layout matches visuals exactly.
 */
public final class TouchControlsOverlay extends InputAdapter {
    ...
    /** Teleport only when touch Y is strictly above this (blocks button row + gaps). */
    ...
    public TouchControlsOverlay(Main game, Player player, Runnable onPause) {
        this.game = game;
        this.player = player;
    ...
    public void layout() {
        ...
        float size = UiFactory.touchButtonSize(h);
        float pad = UiSpacing.small(h);
        float bottomY = UiBounds.safeBottom(h);

        float x = UiBounds.safeLeft(h);
        leftBounds.set(x, bottomY, size, size);
        x += size + pad;
        rightBounds.set(x, bottomY, size, size);

        shieldBounds.set(UiBounds.safeRight(w, h) - size, bottomY, size, size);

        // Under top-left health bar (see HUDManager.drawHealthBar)
        float hudPad = UiBounds.margin(h);
        float healthTop = h - hudPad;
        float barH = UiSpacing.pxToWorld(12f, h);
        float barBorder = UiSpacing.pxToWorld(2f, h);
        float belowHealth = healthTop - barH - barBorder - pad;
        float pauseY = UiBounds.clampY(belowHealth - size, size, h);
        float pauseX = UiBounds.safeLeft(h);
        pauseBounds.set(pauseX, pauseY, size, size);

        teleportMinWorldY = bottomY + size + pad;
    }
